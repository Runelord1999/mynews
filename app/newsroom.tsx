...
import {ArrowUpRight,Bookmark,RefreshCw,Settings2,Newspaper,BookOpen,Info,Eye,EyeOff} from 'lucide-react';
import {Tabs,TabsList,TabsTrigger} from '@/components/ui/tabs';
import {Dialog,DialogContent,DialogTitle,DialogDescription} from '@/components/ui/dialog';
import {Select,SelectContent,SelectItem,SelectTrigger,SelectValue} from '@/components/ui/select';
import {Popover,PopoverTrigger,PopoverContent} from '@/components/ui/popover';
import {Toaster,toast} from 'sonner';
import {defaults,safeUrl,allSources,sourceHost,matchTopic,topicQueryGroups,engines,engineIds,type Topic,type Article,type NewsSite} from '@/lib/news';
...
export default function Newsroom(){
 ...
 const feedKey=JSON.stringify({version:4,onlySites,removedSources,sources:[...sources].sort(),topics:topics.map(t=>[t.name,t.keywords]),sites:sites.map(s=>[s.url,s.feedUrl])});
 ...
 if(!jobs.length){setFeed([]);setLoading(false);setNeedsService(searchHosts.length>0);setError(searchHosts.length?'These websites have no full-text feed, so they can only be reached by searching. Turn on a search service to use as the index. Tick “Only from my sites” under Sources to keep results to these websites alone.':'No sources are selected. Choose at least one under Sources.');return;}
