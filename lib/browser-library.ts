...
const librarySchema=z.object({topics:z.array(topicSchema).min(1).max(20),articles:z.array(articleSchema),sites:z.array(siteSchema).max(30).default([])});
...
export function restoreBackup(backup:SettingsBackup) {
 ...
 localStorage.setItem(key,JSON.stringify({topics:valid.topics,articles:valid.articles,sites:valid.sites}));
 return valid;
}
export function readLibrary():{topics:Topic[];articles:Article[];sites:NewsSite[]}{const raw=localStorage.getItem(key);return raw?librarySchema.parse(JSON.parse(raw)):{topics:defaults,articles:[],sites:[]};}
